...
```{r}
setwd("//Users//priyavivekbhandarkar//Desktop//INSOFE//ALL MODULES INSOFE/CUTE 3")
Train_Bankrupts <- read.csv("train.csv")
Test_Bankrupts <- read.csv("test.csv")
```
...
* The dataset has 31283 observations and 66 variables
* Two variables are integers and others variables are numeric
* Target variable have binary (0,1) value.
...
```{r}
summary(Train_Bankrupts)
```

```{r}
View(Train_Bankrupts)
```
 * The dataset has 31283 rows and 66 columns
...
```{r}
head(Train_Bankrupts)
```

```{r}
tail(Train_Bankrupts)
```
# Data Description
...
plot(Train_Bankrupts[,"Attr9"],Train_Bankrupts[,"Attr10"],xlab="sale / total assets",ylab="equity / total assets",type="p",main="sales and equity" )
plot(Train_Bankrupts[,"Attr18"],Train_Bankrupts[,"Attr24"],xlab="gross profit / total assets",ylab="gross profit (in 3 years) / total assets",type="p",main="gross profit now and in 3 years" )
plot(Train_Bankrupts[,"Attr22"],Train_Bankrupts[,"Attr7"],xlab="profit on operating activities / total assets ",ylab="EBIT / total assets",type="p",main="EBIT and profit on operating activities" )
plot(Train_Bankrupts[,"Attr2"],Train_Bankrupts[,"Attr3"],xlab="total liabilities / total assets",ylab="working capital / total assets",type="p",main="working capital and total liabilities" )
```
# Check for missing values

```{r}
sum(is.na(Train_Bankrupts))
```

```{r}
Train_Bankrupts_Impute=centralImputation(Train_Bankrupts)
sum(is.na(Train_Bankrupts_Impute))
```
# Check for class imbalance

```{r}
prop.table(table(Train_Bankrupts_Impute$target))

```
## Find the corelation between the features.
```{r}
cat_var="target"
num_var=setdiff(names(Train_Bankrupts_Impute),cat_var)
corrplot( cor(Train_Bankrupts_Impute[,num_var]), method="circle", tl.cex = 0.5, tl.col = 'black',  order = "hclust", diag = FALSE)
```

```{r}
cor(Train_Bankrupts_Impute)
```
...
```{r}
sum(is.na(Test_Bankrupts))

```

```{r}
Test_Bankrupts_Impute=centralImputation(Test_Bankrupts)
sum(is.na(Test_Bankrupts_Impute))
```
* EXPLORE THE Attr1

```{r}
boxplot (Attr26 ~ target, data = Train_Bankrupts_Impute, 
         main = "Attr26 distribution for bankrupt and not bankrupt ",
         xlab = "target", ylab = "Attr1", col = "salmon")
```
## EXPLORE THE Attr3

```{r}
summary (Train_Bankrupts_Impute$Attr3)

```

```{r}
boxplot (Attr3 ~ target, data = Train_Bankrupts_Impute, 
         main = "Attr3 distribution for bankrupt and not bankrupt",
         xlab = "target", ylab = "Attr3", col = "blue")
...
trainIndex <- createDataPartition(Train_Bankrupts_Impute$target, p = .7, list = F)

train_data <- Train_Bankrupts_Impute[trainIndex, ]

test_data <- Train_Bankrupts_Impute[-trainIndex, ]#Train_Bankrupts_Impute
```
...
```
# Observe that there is class imbalance in the data -even if you put Premium for all samples .
...
```{r}
as.data.frame(table(trainsmote$target))

```
# Build model
* 1)logistic regression
```{r}
log_reg <- glm(target ~., data = trainsmote, family = binomial)
...
preds_test1 <- ifelse(prob_test1 >= 0.7, "1", "0")
```

```{r}
preds_test1<-data.frame(preds_test1)
confusionMatrix(preds_test1$preds_test1, test_data$target)
```
...
```{r}
pred <- prediction(prob_train, trainsmote$target)
```
...
plot(perf, col = rainbow(10), colorize=T, main="ROC plot",   print.cutoffs.at=seq(0,1,0.05))
#####################
# List of predictions
...
```
## 2) Build the model using SVM

```{r}
#SVM
svm_model <- svm(target ~ ., data = trainsmote)
svm_p <- predict(svm_model, test_data[,-66])
accuracy <- sum(svm_p==test_data$target)/length(svm_p)
accuracy
```

#3) Build a Decision Tree

* It will be using Quinlan???s C5.0 decision tree algorithm implementation from the C50 package to build our decision tree

```{r}
c5_tree <- C5.0(target ~ . , trainsmote)

# Use the rules = T argument if you want to extract rules later from the model

c5_rules <- C5.0(target ~ . , trainsmote, rules = T)
```
...
```{r}
preds <- predict(c5_tree, trainsmote)
...
```{r}
conf_train=confusionMatrix(preds, trainsmote$target, positive = "0")
...
*since it is very important to have high precision and recall for this problem
*Print F1 score
```{r}
...
```{r}
reg_tree <- rpart(target ~ ., trainsmote,method='class')
...
```{r}
pred2 = predict(reg_tree, trainsmote,type = "class")
pred3 = predict(reg_tree, test_data,type = "class")

table(pred2, trainsmote$target)
```
...
* can be accessed through the ???confusionMatrix()??? function from the caret package
```{r}
conf_train=confusionMatrix(pred2, trainsmote$target, positive = "0")
...
```{r}
  # N = 1/3/5/7
  Neigh <-3
  pred=knn(trainsmote[,num_var], test_data[,num_var], trainsmote$target , k = Neigh)
...
accu
```
## 4) Model Building using Random Forest and tuning
```{r}
# Build the classification model using randomForest
model = randomForest(target ~ ., data=trainsmote, 
...
```
#Extract and store important variables 
...
pred_Train_rd = predict(model, 
                     trainsmote[,setdiff(names(trainsmote), "target")],
                     type="response", 
                     norm.votes=TRUE)
```
# Build confusion matrix and find accuracy
```{r}
cm_Train = table("actual"= trainsmote$target, "predicted" = pred_Train_rd);
...
```{r}
pred_Train_rd_attr = predict(model_Imp, trainsmote[,top_Imp_Attr],
                     type="response", norm.votes=TRUE)
```
# Build confusion matrix and find accuracy
```{r}
cm_Train = table("actual" = trainsmote$target, 
...
top_Imp_Attr = as.character(rf_Imp_Attr$Attributes[1:9])
set.seed(123)
x <- trainsmote[,!(names(trainsmote) %in% c("target"))]
y <- trainsmote[,(names(trainsmote) %in% c("target"))]
str(y)
```
...
```{r}
pred_Train_rd_tune = predict(tunedmodel, trainsmote,
                     type="response", norm.votes=TRUE)
...
can be accessed through the ???confusionMatrix()??? function from the caret package
```{r}
conf_train_rd=confusionMatrix(pred_Train_rd, trainsmote$target, positive = "0")
conf_test_rd=confusionMatrix(pred_Test_rd, test_data$target, positive = "0")
```

```{r}
conf_train_rd_attr=confusionMatrix(pred_Train_rd_attr, trainsmote$target, positive = "0")
conf_test_rd_attr=confusionMatrix(pred_Test_rd_attr, test_data$target, positive = "0")
```

```{r}
conf_train_rd_tune=confusionMatrix(pred_Train_rd_tune, trainsmote$target, positive = "0")
...
######################
# Write the final prediction to submission.csv
...
# tunedmodel
set.seed(456)
index_value = predict(tunedmodel,Test_Bankrupts_Impute[,setdiff(names(test_data),"target")], type="response", norm.votes=TRUE)
View(index_value)

```

```{r}
submission$prediction<-index_value 
write.csv(submission,file = "priya_sub_R_1.csv",row.names = FALSE)
```


```{r}
a=read.csv("Priya_python23.csv")
```
...
```{r}
prop.table(table(a$prediction))
```

```{r}
b=read.csv("submission.csv")
```
